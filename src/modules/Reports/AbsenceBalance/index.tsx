...
const MONTH_KEYS = [
  "reports.absence_balance.month_january",
  "reports.absence_balance.month_february",
  "reports.absence_balance.month_march",
  "reports.absence_balance.month_april",
  "reports.absence_balance.month_may",
  "reports.absence_balance.month_june",
  "reports.absence_balance.month_july",
  "reports.absence_balance.month_august",
  "reports.absence_balance.month_september",
  "reports.absence_balance.month_october",
  "reports.absence_balance.month_november",
  "reports.absence_balance.month_december",
] as const;

// ponytail: translate() instead of t() — called from module-level helpers; it re-reads
// the locale on every render, so a language switch still updates the label.
...
const formatCyclePeriod = (item: AbsenceBalanceTableItem): string => {
  ...
  if (item.period === "month") {
    return `${monthName(Number(month) - 1)} ${year}`.trim();
  }
  if (item.period === "week") {
    return `${formatIsoDateRu(item.cycle_from)} – ${formatIsoDateRu(item.cycle_to)}`;
  }
  return `${year} ${translate("reports.absence_balance.year_suffix")}`;
};

type RowFilter = "" | "pending" | "exhausted";

const ROW_FILTER_OPTIONS: { value: RowFilter; labelKey: MessageKey }[] = [
  { value: "", labelKey: "reports.absence_balance.filter_all_rows" },
  { value: "pending", labelKey: "reports.absence_balance.filter_pending" },
  { value: "exhausted", labelKey: "reports.absence_balance.filter_exhausted" },
];
  ...
  return (
    <article className="rounded-2xl border border-gray-200 bg-white p-4">
      ...
      <div className="mt-3 flex items-end justify-between">
        <div>
          <p className="text-[11px] text-gray-400">{t("reports.absence_balance.used_in_cycle")}</p>
          <p className="mt-0.5 text-lg font-semibold text-gray-900">
            {formatMetric(policy.used_days)}
            <span className="text-xs font-medium text-gray-400">
              {" "}
              / {formatMetric(policy.total_limit)} {t("reports.absence_balance.days_short")}
            </span>
          </p>
        </div>
      ...
      <div className="mt-3 flex items-center justify-between text-[11px]">
        <span className="text-gray-500">
          {policy.employees_count} {t("reports.absence_balance.employees_short")} ·{" "}
          {t("reports.absence_balance.available_short")} {formatMetric(policy.available_days)}{" "}
          {t("reports.absence_balance.days_short")}
        </span>
        <span className="flex items-center gap-2">
          {policy.pending_requests > 0 ? (
            <span className="font-semibold text-amber-600">
              {policy.pending_requests} {t("reports.absence_balance.pending_short")}
            </span>
          ) : null}
          {policy.exhausted_count > 0 ? (
            <span className="font-semibold text-rose-600">
              {policy.exhausted_count} {t("reports.absence_balance.exhausted_short")}
            </span>
          ) : null}
        </span>
      </div>
...
function AbsenceBalancePage() {
  ...
  const periodLabel = `${monthName(periodMonth)} ${periodYear}`;
  ...
  const departmentChartSeries = useMemo(
    () => [
      { name: t("reports.absence_balance.chart_used"), data: byDepartment.map((item) => item.used_days) },
      { name: t("reports.absence_balance.chart_pending"), data: byDepartment.map((item) => item.pending_days) },
    ],
    [byDepartment, t]
  );
  ...
  const departmentChartOptions: ApexOptions = useMemo(
    () => ({
      ...
      yaxis: {
        title: { text: t("reports.absence_balance.days_axis"), style: { fontSize: "12px", color: "#64748b" } },
  ...
  if (isLoading) {
    return (
      <>
        <PageMeta
          title={t("reports.absence_balance.page_title")}
          description={t("reports.absence_balance.page_description")}
        />
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-gray-500">
            {t("reports.absence_balance.report_period_label")}{" "}
          ...
          <div className="inline-flex items-center overflow-hidden rounded-xl border border-gray-200 bg-white">
            <button
              ...
              aria-label={t("reports.absence_balance.prev_month_aria")}
            >
              <ChevronLeft size={16} />
            </button>
            ...
            <button
              ...
              aria-label={t("reports.absence_balance.next_month_aria")}
            >
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryCard
            icon={<Users size={18} />}
            title={t("reports.absence_balance.card_employees")}
            value={String(Number(cards.total_employees || 0))}
            hint={`${Number(cards.total_policies || 0)} ${t("reports.absence_balance.card_absence_types_hint")}`}
            accent="#3b82f6"
          />
          <SummaryCard
            icon={<Clock size={18} />}
            title={t("reports.absence_balance.card_pending_approval")}
            value={String(Number(cards.pending_requests || 0))}
            hint={`${formatMetric(Number(cards.pending_days || 0))} ${t("reports.absence_balance.card_days_in_requests_hint")}`}
            accent="#f59e0b"
          />
          <SummaryCard
            icon={<CalendarClock size={18} />}
            title={`${t("reports.absence_balance.card_used_for")} ${reportYear}`}
            value={`${formatMetric(Number(cards.used_days_year || 0))} ${t("reports.absence_balance.days_short")}`}
            hint={t("reports.absence_balance.card_used_year_hint")}
            accent="#10b981"
          />
          <SummaryCard
            icon={<AlertTriangle size={18} />}
            title={t("reports.absence_balance.card_exhausted")}
            value={String(Number(cards.exhausted_count || 0))}
            hint={t("reports.absence_balance.card_exhausted_hint")}
            accent="#ef4444"
          />
        </section>
        ...
        {byPolicy.length > 0 ? (
          <section>
            <h3 className="mb-2 px-1 text-sm font-semibold text-gray-700">
              {t("reports.absence_balance.usage_by_type_title")}
            </h3>
        ...
        {byDepartment.length > 0 ? (
          <section className="rounded-2xl border border-gray-200 bg-white px-4 py-4">
            <h3 className="text-sm font-semibold text-gray-700">
              {t("reports.absence_balance.used_pending_by_department_title")}
            </h3>
            <div className="mt-2">
              <Chart
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label className="relative w-full sm:max-w-xs">
                ...
                <input
                  ...
                  placeholder={t("reports.absence_balance.search_employee_placeholder")}
              ...
              >
                <option value="all">{t("reports.absence_balance.all_absence_types_option")}</option>
        ...
        <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
          <div className="border-b border-gray-100 bg-gray-50/60 px-4 py-2.5">
            <p className="text-xs leading-relaxed text-gray-500">
              {t("reports.absence_balance.table_note_prefix")}{" "}
              <span className="font-semibold text-gray-600">
                {t("reports.absence_balance.table_note_current_cycle")}
              </span>{" "}
              {t("reports.absence_balance.table_note_suffix")} {reportYear}{" "}
              {t("reports.absence_balance.table_note_used_suffix")}
            </p>
          </div>
            ...
            <table className="min-w-full border-separate border-spacing-0">
              <thead>
                <tr className="bg-gray-50">
                  {[
                    t("reports.absence_balance.col_full_name"),
                    t("reports.absence_balance.col_absence_type"),
                    t("reports.absence_balance.col_period"),
                    t("reports.absence_balance.col_limit"),
                    `${t("reports.absence_balance.card_used_for")} ${reportYear}`,
                    t("reports.absence_balance.col_pending"),
                    t("reports.absence_balance.col_available"),
                    t("reports.absence_balance.col_utilization"),
                  ].map((column) => (
                    <th
                      key={column}
