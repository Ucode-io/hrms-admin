...
const eventType = (actionValue: unknown): AccessEvent["type"] => {
  const raw = Array.isArray(actionValue) ? readString(actionValue[0]) : readString(actionValue);
  const action = raw.toLocaleLowerCase();
  ...
  return (
    <div className="min-w-[142px] rounded-lg border border-slate-200 bg-white px-2.5 py-1.5">
      ...
      <div className="mt-1.5 flex items-center gap-2">
        <div className="relative h-1 flex-1 rounded-full bg-slate-200" title={`${workedMinutes} минут, план 480 минут`}>
          <div className="absolute inset-y-0 left-0 rounded-full bg-emerald-500" style={{ width: `${progress}%` }} />
          <span className="absolute top-1/2 h-3 w-0.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-slate-900" style={{ left: `${planPosition}%` }} aria-label="План 8 часов" />
        </div>
...
function MissingEventIcon({ type }: { type: "in" | "out" }) {
  ...
  return (
    <span className="relative inline-flex h-[18px] w-[18px] items-center justify-center" role="img" aria-label={type === "in" ? "Приход не отмечен" : "Уход не отмечен"}>
...
function EmployeesOverview({
  ...
  if (error) return <EmptyState icon="clock" title="Не удалось загрузить события Hikvision" hint="Проверьте подключение интеграции и повторите попытку" />;
  if (employees.length === 0) return <EmptyState icon="users" title="За этот месяц событий Hikvision нет" hint="Сотрудники без реальных входов и выходов не показываются" />;
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-end border-b border-slate-100 bg-slate-50/70 px-4 py-2.5">
        <div className="inline-flex h-[38px] items-center rounded-xl border border-slate-200 bg-white p-[3px]">
          <button type="button" aria-label="Предыдущий месяц" onClick={() => onShiftMonth(-1)} className="inline-flex h-[30px] w-[30px] items-center justify-center rounded-lg text-slate-600 transition hover:border-slate-200 hover:bg-slate-50"><ChevronLeft size={16} /></button>
          <span className="min-w-[170px] px-3 text-center text-[13px] font-semibold text-slate-700">{monthLabel}</span>
          <button type="button" aria-label="Следующий месяц" onClick={() => onShiftMonth(1)} className="inline-flex h-[30px] w-[30px] items-center justify-center rounded-lg text-slate-600 transition hover:border-slate-200 hover:bg-slate-50"><ChevronRight size={16} /></button>
        </div>
      </div>
      <div ref={scrollRef} className="max-w-full overflow-x-auto">
        <table className="min-w-full border-collapse">
          <thead className="border-b border-slate-100 bg-slate-50/70">
            <tr>
              <th className="sticky left-0 z-10 min-w-[285px] bg-slate-50 px-5 py-2.5 text-left text-xs font-semibold text-slate-500">Сотрудник</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {employees.map((employee) => (
              <tr
                ...
                role="button"
                tabIndex={0}
                aria-label={`Открыть события Hikvision сотрудника ${employee.name}`}
      ...
      <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3 text-xs text-slate-400"><span>Показаны только сотрудники с отметками Hikvision</span><span className="font-semibold text-slate-500">{employees.length} сотрудников</span></div>
    </div>
  );
}
        ...
        <div className="absolute left-0 right-0 top-7 h-12 overflow-hidden rounded-xl border border-slate-100 bg-slate-50">
          {hasAttendanceSpan ? (
            <span
              ...
              title={`С ${firstEntry?.event.time} до ${lastAction.event.time}`}
            >
